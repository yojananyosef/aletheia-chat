...
- `src/` contiene el dominio: `views/`, `components/`, `hooks/`, `context/`, `core/` (`domain/Message.ts` es la fuente de verdad de hablantes y reglas de avance) y `constants/books.ts` (manifiesto del canon: 66 libros, 51 desbloqueados).
...
Estado del corpus: **51 / 66 libros publicados** (1.111 capítulos). Pendiente el resto del
Nuevo Testamento (15 libros, 78 capítulos) más los desbloqueos en `books.ts`.
