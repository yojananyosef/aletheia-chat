...
- `src/` contiene el dominio: `views/`, `components/`, `hooks/`, `context/`, `core/` (`domain/Message.ts` es la fuente de verdad de hablantes y reglas de avance) y `constants/books.ts` (manifiesto del canon: 66 libros, 50 desbloqueados).
...
Estado del corpus: **50 / 66 libros publicados** (1.107 capítulos). Pendiente el resto del
Nuevo Testamento (16 libros, 82 capítulos) más los desbloqueos en `books.ts`.
