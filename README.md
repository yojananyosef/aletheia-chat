...
- `src/` contiene el dominio: `views/`, `components/`, `hooks/`, `context/`, `core/` (`domain/Message.ts` es la fuente de verdad de hablantes y reglas de avance) y `constants/books.ts` (manifiesto del canon: 66 libros, 46 desbloqueados).
...
Estado del corpus: **46 / 66 libros publicados** (1.078 capítulos). Pendiente el resto del
Nuevo Testamento (20 libros, 111 capítulos) más los desbloqueos en `books.ts`.
