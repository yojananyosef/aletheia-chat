...
- `src/` contiene el dominio: `views/`, `components/`, `hooks/`, `context/`, `core/` (`domain/Message.ts` es la fuente de verdad de hablantes y reglas de avance) y `constants/books.ts` (manifiesto del canon: 66 libros, 57 desbloqueados).
...
Estado del corpus: **57 / 66 libros publicados** (1.133 capítulos). Pendiente el resto del
Nuevo Testamento (9 libros, 60 capítulos) más los desbloqueos en `books.ts`.
