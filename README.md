...
- `src/` contiene el dominio: `views/`, `components/`, `hooks/`, `context/`, `core/` (`domain/Message.ts` es la fuente de verdad de hablantes y reglas de avance) y `constants/books.ts` (manifiesto del canon: 66 libros, 53 desbloqueados).
...
Estado del corpus: **53 / 66 libros publicados** (1.119 capítulos). Pendiente el resto del
Nuevo Testamento (13 libros, 70 capítulos) más los desbloqueos en `books.ts`.
