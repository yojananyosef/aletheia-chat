...
- `src/` contiene el dominio: `views/`, `components/`, `hooks/`, `context/`, `core/` (`domain/Message.ts` es la fuente de verdad de hablantes y reglas de avance) y `constants/books.ts` (manifiesto del canon: 66 libros, 60 desbloqueados).
...
Estado del corpus: **60 / 66 libros publicados** (1.156 capítulos). Pendiente el resto del
Nuevo Testamento (6 libros, 33 capítulos) más los desbloqueos en `books.ts`.
