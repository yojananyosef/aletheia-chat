...
- `src/` contiene el dominio: `views/`, `components/`, `hooks/`, `context/`, `core/` (`domain/Message.ts` es la fuente de verdad de hablantes y reglas de avance) y `constants/books.ts` (manifiesto del canon: 66 libros, 58 desbloqueados).
...
Estado del corpus: **58 / 66 libros publicados** (1.146 capítulos). Pendiente el resto del
Nuevo Testamento (8 libros, 48 capítulos) más los desbloqueos en `books.ts`.
