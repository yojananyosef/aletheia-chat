...
- `src/` contiene el dominio: `views/`, `components/`, `hooks/`, `context/`, `core/` (`domain/Message.ts` es la fuente de verdad de hablantes y reglas de avance) y `constants/books.ts` (manifiesto del canon: 66 libros, 47 desbloqueados).
...
Estado del corpus: **47 / 66 libros publicados** (1.091 capítulos). Pendiente el resto del
Nuevo Testamento (19 libros, 98 capítulos) más los desbloqueos en `books.ts`.
