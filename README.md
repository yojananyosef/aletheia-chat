...
- `src/` contiene el dominio: `views/`, `components/`, `hooks/`, `context/`, `core/` (`domain/Message.ts` es la fuente de verdad de hablantes y reglas de avance) y `constants/books.ts` (manifiesto del canon: 66 libros, 52 desbloqueados).
...
Estado del corpus: **52 / 66 libros publicados** (1.116 capítulos). Pendiente el resto del
Nuevo Testamento (14 libros, 73 capítulos) más los desbloqueos en `books.ts`.
