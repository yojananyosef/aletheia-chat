...
- `src/` contiene el dominio: `views/`, `components/`, `hooks/`, `context/`, `core/` (`domain/Message.ts` es la fuente de verdad de hablantes y reglas de avance) y `constants/books.ts` (manifiesto del canon: 66 libros, 48 desbloqueados).
...
Estado del corpus: **48 / 66 libros publicados** (1.097 capítulos). Pendiente el resto del
Nuevo Testamento (18 libros, 92 capítulos) más los desbloqueos en `books.ts`.
