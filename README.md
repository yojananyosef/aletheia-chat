...
- `src/` contiene el dominio: `views/`, `components/`, `hooks/`, `context/`, `core/` (`domain/Message.ts` es la fuente de verdad de hablantes y reglas de avance) y `constants/books.ts` (manifiesto del canon: 66 libros, 56 desbloqueados).
...
Estado del corpus: **56 / 66 libros publicados** (1.132 capítulos). Pendiente el resto del
Nuevo Testamento (10 libros, 61 capítulos) más los desbloqueos en `books.ts`.
