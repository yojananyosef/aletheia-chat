...
- `src/` contiene el dominio: `views/`, `components/`, `hooks/`, `context/`, `core/` (`domain/Message.ts` es la fuente de verdad de hablantes y reglas de avance) y `constants/books.ts` (manifiesto del canon: 66 libros, 45 desbloqueados).
...
Estado del corpus: **45 / 66 libros publicados** (1.062 capítulos). Pendiente el resto del
Nuevo Testamento (21 libros, 127 capítulos) más los desbloqueos en `books.ts`.
