...
export const InputBar: React.FC<InputBarProps> = (props) => {
    return (
        <div className="border-t-4 border-[#0A0A0A] bg-white/95 backdrop-blur-md px-4 py-4 sm:p-6 flex justify-center z-50 shrink-0 h-auto pb-safe transition-all shadow-[0_-4px_10px_rgba(0,0,0,0.05)] select-none dark:bg-[#141414]/95 dark:border-white">
                ...
                {!props.error && !props.isComplete ? (
                    !props.nextMessage ? (
                        ...
                        <button
                            ...
                            className="w-full bg-[#FAFAFA] text-[#0A0A0A] border-2 border-black font-black uppercase text-[11px] md:text-lg shadow-[3px_3px_0_#0A0A0A] flex items-center justify-center p-3 text-center active:scale-95 transition-all"
                        >
                        ...
                        <>
                            ...
                            >
                                ...
                                <button
                                    ...
                                    className="bg-[#FFD600] text-[#0A0A0A] w-14 h-14 rounded-full flex items-center justify-center border-2 border-black shadow-[3px_3px_0_#0A0A0A] active:scale-90 transition-all outline-none shrink-0"
                                >
                                <Send className="w-6 h-6" />
                            </button>
                        </>
                    ) : (
                        <div className={`flex-1 h-full flex items-center justify-center font-black uppercase tracking-[0.2em] italic animate-pulse text-[9px] md:text-sm ${props.nextMessage?.speaker === 'Serpiente' ? 'text-red-500' : 'text-gray-600 dark:text-gray-300'
                            }`}>
                    ...
                    <div className="w-full flex items-center gap-3">
                        {props.hasPrev && (
                            <button
                                ...
                                className="py-4 px-4 bg-white text-[#0A0A0A] border-4 border-black font-black text-base md:text-2xl uppercase shadow-[6px_6px_0_#0A0A0A] flex items-center justify-center active:scale-95 transition-all shrink-0 dark:border-white"
                            >
                        ...
                        <button
                            ...
                            className="flex-1 py-4 bg-[#FFD600] text-[#0A0A0A] border-4 border-black font-black text-base md:text-2xl uppercase shadow-[6px_6px_0_#0A0A0A] flex items-center justify-center active:scale-95 transition-all"
                        >
